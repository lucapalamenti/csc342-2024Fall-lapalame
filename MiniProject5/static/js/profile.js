...
// Responsible for deciding whether the follow button should say follow or unfollow
...
api.getUserById( id ).then( returnedUser => {
    ...
    topBar.style.backgroundImage = `url(${returnedUser["profile_banner"]})`;
    api.getCurrentUser().then( returnedCurrentUser => {
...
api.getFollowedUsersIds( id ).then( returnedIdList => {
    api.getCurrentUser().then( currentUser => {
        returnedIdList.forEach( userId => {
            // List the profile's following list, excluding the current logged in user, and the profile's user themself
            if ( userId != currentUser.id && userId != id ) {
                ...
                api.getUserById( userId ).then( returnedUser => {
                    ...
                    followsBanner.style.backgroundImage = `url(${returnedUser["profile_banner"]})`;
